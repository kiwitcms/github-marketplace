...
""".strip() % (
        ...
        signature = github.calculate_signature(
            settings.KIWI_GITHUB_MARKETPLACE_SECRET, json.dumps(payload).encode()
        )
    ...
    def test_visit_not_logged_in(self):
        ...
        self.assertRedirects(
            response, reverse("tcms-login") + f"?next={self.create_tenant_url}"
        )
...
""".strip() % (
        ...
        self.client.login(
            username=self.tenant.owner.username, password="password"  # nosec:B106
        )
